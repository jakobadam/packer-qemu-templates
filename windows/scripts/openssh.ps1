...
$sshd_config = $sshd_config -replace '#UsePrivilegeSeparation yes', 'UsePrivilegeSeparation yes'
...
$majorOSVersion = [System.Environment]::OSVersion.Version.Major
if ($majorOSVersion -eq 5) {
  netsh firewall add allowedprogram name="sshd" mode=ENABLE program="%ProgramFiles(x86)%\OpenSSH\usr\sbin\sshd.exe"
  netsh firewall add portopening name="ssh" mode=ENABLE protocol=TCP port=22
} else {
